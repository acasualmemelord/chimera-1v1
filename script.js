...
function importTeam(){
  ...
  for(var j = 0; j < 6; j ++){
    try{
      ...
      i = team[j].indexOf("\n", i + 1) + 3;
      document.getElementById("sortable").children[j].children[9].value = team[j].substring(i, team[j].indexOf("  ", i + 1));
    }catch(err){
...
function importSet(){
  var team = document.getElementById("setpaste");
  var i = 0;
  var j = document.getElementById("importmon");
    try {
      document.getElementById("sortable").children[j].children[0].value = team[j].substring(0, team[j].indexOf("@") - 1);
      i = team[j].indexOf("@") + 2;
      document.getElementById("sortable").children[j].children[1].value = team[j].substring(i, team[j].indexOf("\n") - 2);
